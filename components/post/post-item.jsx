...
export default function PostItem({ post, className = '', styles, config = {} }) {
    if (!post) {
        return (null)
    }
    ...
    const classes = {
        ...
        innerWrapper: {
            first: conf.listView && conf.showImage ? 'basis-1/3' : '',
            second: conf.listView && conf.showImage ? 'basis-2/3 px-4' : ''
        },
        image: {
    ...
    return <div className={`post-${databaseId}`}>
