import {
  ...
  QUERY_CATEGORIES_WITH_PAGINATED_POSTS_BY_SLUGS,
 ...
 * getAllCategoriesPath
 */
export async function getCategoriesWithPaginatedPostsBySlugs(slugs = []) {
  const apolloClient = initializeApollo();

  const data = await apolloClient.query({
    query: QUERY_CATEGORIES_WITH_PAGINATED_POSTS_BY_SLUGS,
    variables: {
      slugs,
      first: ARCHIVE_POST_PER_PAGE
    },
  });
