...
import {
  ...
  QUERY_POSTS_BY_CATEGORY_TAG,
  QUERY_POST_SEO_BY_ID,
  QUERY_POST_PER_PAGE,
...
export async function getPostById(databaseId) {
  ...
  const apiHost = new URL(process.env.WORDPRESS_API_URL).host;
  ...
  try {
    ...
    console.warn(`[posts][getPostBySlug] Failed to query post data: ${e.message}`);
    throw e;
  }
  ...
  if (process.env.WORDPRESS_PLUGIN_SEO === true) {
    try {
      seoData = await apolloClient.query({
        query: QUERY_POST_SEO_BY_ID,
        variables: {
          id: databaseId,
        },
      });
    } catch (e) {
      console.warn(`[posts][getPostBySlug] Failed to query SEO plugin: ${e.message}`);
      console.warn('Is the SEO Plugin installed? If not, disable WORDPRESS_PLUGIN_SEO in next.config.js.');
      throw e;
    }
...
export async function getPostsByAuthorSlug({ slug, ...options }) {
  ...
  try {
    ...
    console.warn(`[posts][getPostsByAuthorSlug] Failed to query post data: ${e.message}`);
    throw e;
  }
...
export async function getPostsByCategoryId({ categoryId, ...options }) {
  ...
  try {
    ...
    console.warn(`[posts][getPostsByCategoryId] Failed to query post data: ${e.message}`);
    throw e;
  }
...
export async function getPostsByTagSlug({ slug }) {
  ...
  try {
    postData = await apolloClient.query({
      query: QUERY_POSTS_BY_CATEGORY_TAG,
      variables: {
        slug,
      },
    });
