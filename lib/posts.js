...
export async function getPostById(databaseId, seo = false) {
  const apolloClient = initializeApollo();
  let postData;
  try {
    postData = await apolloClient.query({
      query: seo ? QUERY_POST_SEO_BY_ID : QUERY_POST_BY_ID,
      variables: {
        id: databaseId,
      },
    });
...
export async function getPostsSearch({ search, first = 5 }) {

  const apolloClient = initializeApollo();
  
  const queryData = await apolloClient.query({
    query: QUERY_POSTS_SEARCH,
    variables: {
      search,
      first
    }
